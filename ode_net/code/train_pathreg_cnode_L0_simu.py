...
if __name__ == "__main__":
    ...
    model_type = "pathreg"  # or "cnode" or "L0"

    if model_type == "pathreg":
        lambda_l0 = 1
        lambda_l1 = 0.001
        lambda_pathreg = 0.0001
        my_temp = 2/3
    elif model_type == "cnode":
        lambda_l0 = 0  
        lambda_l1 = 0.001
        lambda_pathreg = 0
        my_temp = 2/3
        ...
        print("Invalid model_type. Please choose from 'pathreg', 'cnode', or 'L0'")
    
    
    

    data_dim = data_handler.dim
    nhidden = settings['method']
    feature_layers = [initial_position(data_dim, nhidden), 
                    ODEBlock(
                        odefunc = L0_MLP(input_dim=data_dim, layer_dims=(nhidden,nhidden,data_dim), temperature = my_temp, my_lambda = lambda_l0), 
                        dim = data_dim, 
                        tol = 1e-3,
    ...
    with open('{}/network.txt'.format(output_root_dir), 'w') as net_file:
        ...
        print(".......", file = net_file)
        print("model_type = {}".format(model_type), file = net_file)
        print(".......", file = net_file)
        print("lambda_l0 = {}".format(lambda_l0), file = net_file)
