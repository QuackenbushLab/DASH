...
def setOptimizerLRScheduler(patience = 3):
...
clean_name =  "simu_350genes_150samples_train_val" 
...
if __name__ == "__main__":
    ...
    # READ IN PRIOR MATRIX AND CREATE PPI MATRIX
    prior_mat_loc = '/home/ubuntu/lottery_tickets_phoenix/ground_truth_simulator/clean_data/edge_prior_matrix_simu_350_noise_{}.csv'.format(settings['noise'])
    absolute_flag = False #this determines if all non-zero edge signs should be changed to positive
    ...
    # PARAMETERS THAT GUIDE THE WEIGHT GIVEN TO DATA_LOSS TERM IN PHOENIX; PRIOR_LOSS = 1 - DATA_LOSS
    loss_lambda = 1

    # %PARAMS FORCED TO ZERO WHEN LOADING IN A PRE-TRAINED MODEL (FOR POST-HOC MAGNITUDE PRUNING + FINE-TUNING)
    # If using this, put the pre-trained model files in \output\_holder_model_to_prune 
    prop_force_to_zero_for_loaded_model = 0

    # PARAMETERS FOR IMP/DASH
...
#       opt = optim.Adam(odenet.parameters(), lr=settings['init_lr'], weight_decay=settings['weight_decay'])
    ...
    scheduler = setOptimizerLRScheduler()


    #  THIS LOOP IS USED TO HANDLE ANY POST-HOC MAGNITUDE PRUNING 
    ...
    for epoch in range(1, tot_epochs + 1):
        ...
        #  THIS LOOP IS USED TO HANDLE ANY ITERATIVE PRUNING (THIS CAN BE EITHER IMP OR DASH) 
